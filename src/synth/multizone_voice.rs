...
impl MultiZoneSampleVoice {
    ...
    fn apply_filter_generators(&mut self, _preset: &SoundFontPreset) -> Result<(), AweError> {
        ...
        crate::log(&format!(
            "Filter setup: note={} vel={} → cutoff={:.1}Hz resonance={:.2}",
            self.note, self.velocity, cutoff, resonance
        ));
    ...
    fn apply_effects_send_generators(&mut self, _preset: &SoundFontPreset) -> Result<(), AweError> {
        ...
        crate::log(&format!(
            "Effects sends: note={} vel={} → reverb={:.2} chorus={:.2}",
            self.note, self.velocity, self.reverb_send, self.chorus_send
        ));
