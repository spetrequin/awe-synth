 ...
 */
export function calculatePositionVelocity(relativeY, elementHeight, invert = true) {
 ...
 */
export function calculateTimingVelocity(durationMs, fastThresholdMs = 50, slowThresholdMs = 200) {
    // Fast press (< fastThreshold) = high velocity
    // Slow press (> slowThreshold) = low velocity
    const normalizedDuration = Math.min(Math.max(durationMs - fastThresholdMs, 0) / (slowThresholdMs - fastThresholdMs), 1);
 ...
 */
export function calculatePressureVelocity(pressure, baseVelocity = 0.5) {
 ...
 */
export function calculateContactVelocity(radiusX = 10, radiusY = 10, maxRadius = 50) {
 ...
 */
export function combineVelocityFactors(factors) {
 ...
 */
export function finalizeVelocity(rawVelocity) {
 ...
 */
export function getDefaultVelocityFor(inputType) {
    switch (inputType) {
 ...
 */
export function analyzeVelocityRange(velocities) {
    ...
    for (const velocity of velocities) {
        if (velocity <= 31)
            distribution['pp (0-31)']++;
        else if (velocity <= 47)
            distribution['p (32-47)']++;
        else if (velocity <= 63)
            distribution['mp (48-63)']++;
        else if (velocity <= 79)
            distribution['mf (64-79)']++;
        else if (velocity <= 95)
            distribution['f (80-95)']++;
        else if (velocity <= 111)
            distribution['ff (96-111)']++;
        else
            distribution['fff (112-127)']++;
    }
    return {
 ...
 */
export function createVelocityTestSequence() {
    return [
...
//# sourceMappingURL=velocity-utils.js.map