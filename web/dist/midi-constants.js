...
// Type Guards
export const isValidMidiNote = (note) => note >= MIDI_NOTES.LOWEST_PIANO && note <= MIDI_NOTES.HIGHEST_PIANO;
export const isValidMidiValue = (value) => value >= MIDI_VALUES.MIN && value <= MIDI_VALUES.MAX;
export const isValidMidiChannel = (channel) => channel >= MIDI_CHANNELS.MIN && channel <= MIDI_CHANNELS.MAX;
export const isDrumChannel = (channel) => channel === MIDI_CHANNELS.DRUM_CHANNEL;
// Helper Functions
export const noteToOctave = (note) => Math.floor((note - 12) / MIDI_NOTES.OCTAVE_SIZE);
export const noteToNoteName = (note) => {
    const noteIndex = note % MIDI_NOTES.OCTAVE_SIZE;
    // Type-safe check for white keys
    const whiteKeyPattern = PIANO_LAYOUT.WHITE_KEY_PATTERN;
    ...
    const blackKeyPattern = PIANO_LAYOUT.BLACK_KEY_PATTERN;
...
export const noteToFullName = (note) => {
...
//# sourceMappingURL=midi-constants.js.map