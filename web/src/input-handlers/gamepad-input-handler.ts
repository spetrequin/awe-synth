...
export class GamepadInputHandler extends BaseInputHandler {
    ...
    private lastButtonStates = new Map<number, boolean[]>();
    ...
    private handleGamepadButtonPress(buttonIndex: number, pressure: number): void {
        ...
        if (note < MIDI_NOTES.LOWEST_PIANO || note > MIDI_NOTES.HIGHEST_PIANO) return;
    ...
    private handleGamepadButtonRelease(buttonIndex: number): void {
        const note = this.mapButtonToNote(buttonIndex);
        if (note < MIDI_NOTES.LOWEST_PIANO || note > MIDI_NOTES.HIGHEST_PIANO) return;
    ...
    private processAnalogInputs(gamepad: Gamepad): void {
     ...
     */
    private mapButtonToNote(buttonIndex: number): number {
