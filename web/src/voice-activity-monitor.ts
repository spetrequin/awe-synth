...
export class VoiceActivityMonitor {
    ...
    private voiceElements: HTMLElement[] = [];
    ...
    // Configuration
    private readonly VOICE_COUNT = 32;
    private readonly UPDATE_RATE_MS = 50; // 20 FPS
     ...
     */
    private createVoiceElements(): void {
     ...
     */
    private updateFromDebugLog(): void {
        if (!this.wasmModule.get_debug_log_global) return;
     ...
     */
    private updateFromWASMStates(states: any[]): void {
     ...
     */
    private renderVoiceStates(): void {
        for (let i = 0; i < this.VOICE_COUNT; i++) {
            ...
            // Update note display
            const noteEl = element.querySelector('.voice-note') as HTMLElement;
            if (noteEl && state) {
                noteEl.textContent = state.active ? this.getNoteDisplay(state.note) : '';
            }
            
            // Update amplitude bar
            const barEl = element.querySelector('.voice-bar') as HTMLElement;
            if (barEl && state) {
            ...
            const phaseEl = element.querySelector('.voice-phase') as HTMLElement;
            if (phaseEl && state) {
            ...
            const reverbEl = element.querySelector('.effect-indicator.reverb') as HTMLElement;
            const chorusEl = element.querySelector('.effect-indicator.chorus') as HTMLElement;
     ...
     */
    private getNoteDisplay(midiNote: number): string {
     ...
     */
    private setupMIDIEventListener(): void {
        // Listen for custom MIDI events if UI dispatches them
        window.addEventListener('midi-event', (event: any) => {
