...
export class DebugLogger {
    ...
    private static logElement: HTMLTextAreaElement | null = null;
     ...
     */
    private static getLogElement(): HTMLTextAreaElement | null {
        if (!DebugLogger.logElement) {
            DebugLogger.logElement = document.getElementById('debug-log') as HTMLTextAreaElement;
        }
     ...
     */
    public static createDebugFunction(componentName: string): (message: string) => void {
        const logger = new DebugLogger({ componentName });
        return (message: string) => logger.log(message);
    }
}

// Convenience function for quick debug logging without creating a logger instance
export function debugLog(componentName: string, message: string): void {
    const logger = new DebugLogger({ componentName });
    logger.log(message);
}
